...
export default class GenesisUtil {
  ...
  static isByteExpanded(inputArrayBuffer) {
    // Byte-expanded data is always an even number of bytes (each source byte
    // becomes a 16-bit word). An odd-length buffer therefore cannot be
    // byte-expanded. Returning early here also avoids reading one byte past
    // the end of the DataView on the final (unpaired) iteration below, which
    // throws "Offset is outside the bounds of the DataView" whenever an
    // odd-length buffer looks byte-expanded/uniform all the way through
    // (e.g. an empty save filled with 0x00/0xFF at an odd size).
    if ((inputArrayBuffer.byteLength % 2) !== 0) {
      return false;
    }
